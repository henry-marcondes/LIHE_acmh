...
const prodSelect = document.getElementById('prod_selector');
const consSelect = document.getElementById('cons_selector');
...
function addItem(type) {
    const selectorId = type === 'producer' ? 'prod_selector' : 'cons_selector';
...
function renderItems(type) {
    ...
    data[type].forEach((item, index) => {
        ...
        div.className = 'item';
        div.innerHTML = `
        ${item.name} - Potência: ${item.power}W, Fator: ${item.power_factor}, Espaço: ${item.width}x${item.height}x${item.depth} m
...
function updateResults() {
    ...
    data.producer.forEach(item => {
        totalProd += item.power * item.power_factor;
        supplyTotalArea += item.width * item.height;
        supplyTotalSpace += supplyTotalArea * item.depth;
    });

    data.consumer.forEach(item => {
        totalCons += item.power * item.power_factor;
        sourceTotalArea += item.width * item.height;
        sourceTotalSpace += sourceTotalArea * item.depth;
    });

    document.getElementById('total_prod').textContent = totalProd.toFixed(2);
    document.getElementById('total_cons').textContent = totalCons.toFixed(2);
